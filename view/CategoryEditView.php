...
class CategoryEditView implements ViewInterface
{
    private CategoryRepositoryInterface $rep;
    private int $id;
    public function __construct(CategoryRepositoryInterface $rep,int $id)
    {
        $this->rep = $rep;
        $this->id = $id;
    }
    ...
    {
        $category = $this->rep->findById($this->id);
        ?>
        <div class="row justify-content-center">
            <div class="col-md-8">
                <form action="/db_app/categories/save">
                    <input type="hidden" name="id" value="<?= $category->getId();?>">
                    <div class="input-group mb-3">
                        <div class="input-group-prepend">
                            <span class="input-group-text" id="basic-addon1">Название</span>
                        </div>
                        <input name="name" type="text" value="<?= $category->getName() ?>" class="form-control" placeholder="" aria-label="" aria-describedby="basic-addon1">
                    </div>
                    <div class="input-group mb-3">
                        <div class="input-group-prepend">
